...
@nb.jit
def match_fatjet_pfcand(fatjets, fatjetpfcands, builder):
    # event loop
...
class JetAEProcessor(processor.ProcessorABC):
    def __init__(self):
        ...
        # variables: general
        self._accumulator["tree"]["FatJet_pt"] = np_acc_float()
        self._accumulator["tree"]["FatJet_eta"] = np_acc_float()
        self._accumulator["tree"]["FatJet_phi"] = np_acc_float()
        self._accumulator["tree"]["FatJet_DDX_jetNSecondaryVertices"] = np_acc_int()
        self._accumulator["tree"]["FatJet_DDX_jetNTracks"] = np_acc_int()
        self._accumulator["tree"]["FatJet_DDX_z_ratio"] = np_acc_float()
        self._accumulator["tree"]["FatJet_Proba"] = np_acc_float()
        self._accumulator["tree"]["FatJet_area"] = np_acc_float()
        self._accumulator["tree"]["FatJet_jetId"] = np_acc_int()
        self._accumulator["tree"]["FatJet_lsf3"] = np_acc_float()
        self._accumulator["tree"]["FatJet_mass"] = np_acc_float()
        self._accumulator["tree"]["FatJet_msoftdrop"] = np_acc_float()
        self._accumulator["tree"]["FatJet_rawFactor"] = np_acc_float()
        self._accumulator["tree"]["FatJet_n2b1"] = np_acc_float()
        self._accumulator["tree"]["FatJet_n3b1"] = np_acc_float()
        #self._accumulator["tree"]["FatJet_nBHadrons"] = np_acc_int()
        #self._accumulator["tree"]["FatJet_nCHadrons"] = np_acc_int()

        # variables: tau1
        self._accumulator["tree"]["FatJet_tau1"] = np_acc_float()
        self._accumulator["tree"]["FatJet_DDX_tau1_flightDistance2dSig"] = np_acc_float()
        self._accumulator["tree"]["FatJet_DDX_tau1_trackEtaRel_0"] = np_acc_float()
        self._accumulator["tree"]["FatJet_DDX_tau1_trackEtaRel_1"] = np_acc_float()
        self._accumulator["tree"]["FatJet_DDX_tau1_trackEtaRel_2"] = np_acc_float()
        self._accumulator["tree"]["FatJet_DDX_tau1_trackSip3dSig_0"] = np_acc_float()
        self._accumulator["tree"]["FatJet_DDX_tau1_trackSip3dSig_1"] = np_acc_float()
        self._accumulator["tree"]["FatJet_DDX_tau1_vertexDeltaR"] = np_acc_float()
        self._accumulator["tree"]["FatJet_DDX_tau1_vertexEnergyRatio"] = np_acc_float()
        self._accumulator["tree"]["FatJet_DDX_tau1_vertexMass"] = np_acc_float()

        # variables: tau2
        self._accumulator["tree"]["FatJet_tau2"] = np_acc_float()
        self._accumulator["tree"]["FatJet_DDX_tau2_flightDistance2dSig"] = np_acc_float()
        self._accumulator["tree"]["FatJet_DDX_tau2_trackEtaRel_0"] = np_acc_float()
        self._accumulator["tree"]["FatJet_DDX_tau2_trackEtaRel_1"] = np_acc_float()
        self._accumulator["tree"]["FatJet_DDX_tau2_trackEtaRel_3"] = np_acc_float()
        self._accumulator["tree"]["FatJet_DDX_tau2_trackSip3dSig_0"] = np_acc_float()
        self._accumulator["tree"]["FatJet_DDX_tau2_trackSip3dSig_1"] = np_acc_float()
        self._accumulator["tree"]["FatJet_DDX_tau2_vertexEnergyRatio"] = np_acc_float()
        self._accumulator["tree"]["FatJet_DDX_tau2_vertexMass"] = np_acc_float()

        # variables: tau3 and tau4
        self._accumulator["tree"]["FatJet_tau3"] = np_acc_float()
        self._accumulator["tree"]["FatJet_tau4"] = np_acc_float()

        # variables: track
        self._accumulator["tree"]["FatJet_DDX_trackSip2dSigAboveBottom_0"] = np_acc_float()
        self._accumulator["tree"]["FatJet_DDX_trackSip2dSigAboveBottom_1"] = np_acc_float()
        self._accumulator["tree"]["FatJet_DDX_trackSip2dSigAboveCharm"] = np_acc_float()
        self._accumulator["tree"]["FatJet_DDX_trackSip3dSig_0"] = np_acc_float()
        self._accumulator["tree"]["FatJet_DDX_trackSip3dSig_1"] = np_acc_float()
        self._accumulator["tree"]["FatJet_DDX_trackSip3dSig_2"] = np_acc_float()
        self._accumulator["tree"]["FatJet_DDX_trackSip3dSig_3"] = np_acc_float()

        # variables: subjet 1
        self._accumulator["tree"]["FatJet_subjet1_pt"] = np_acc_float()
        self._accumulator["tree"]["FatJet_subjet1_eta"] = np_acc_float()
        self._accumulator["tree"]["FatJet_subjet1_phi"] = np_acc_float()
        self._accumulator["tree"]["FatJet_subjet1_Proba"] = np_acc_float()
        self._accumulator["tree"]["FatJet_subjet1_mass"] = np_acc_float()
        self._accumulator["tree"]["FatJet_subjet1_tau1"] = np_acc_float()
        self._accumulator["tree"]["FatJet_subjet1_tau2"] = np_acc_float()
        self._accumulator["tree"]["FatJet_subjet1_tau3"] = np_acc_float()
        self._accumulator["tree"]["FatJet_subjet1_tau4"] = np_acc_float()
        self._accumulator["tree"]["FatJet_subjet1_n2b1"] = np_acc_float()
        self._accumulator["tree"]["FatJet_subjet1_n3b1"] = np_acc_float()

        # variables: subjet 2
        self._accumulator["tree"]["FatJet_subjet2_pt"] = np_acc_float()
        self._accumulator["tree"]["FatJet_subjet2_eta"] = np_acc_float()
        self._accumulator["tree"]["FatJet_subjet2_phi"] = np_acc_float()
        self._accumulator["tree"]["FatJet_subjet2_Proba"] = np_acc_float()
        self._accumulator["tree"]["FatJet_subjet2_mass"] = np_acc_float()
        self._accumulator["tree"]["FatJet_subjet2_tau1"] = np_acc_float()
        self._accumulator["tree"]["FatJet_subjet2_tau2"] = np_acc_float()
        self._accumulator["tree"]["FatJet_subjet2_tau3"] = np_acc_float()
        self._accumulator["tree"]["FatJet_subjet2_tau4"] = np_acc_float()
        self._accumulator["tree"]["FatJet_subjet2_n2b1"] = np_acc_float()
        self._accumulator["tree"]["FatJet_subjet2_n3b1"] = np_acc_float()

        self._accumulator["tree"]["FatJet_hadronFlavour"] = np_acc_int()

        # variables: fatjet sv
        self._accumulator["tree"]["FatJet_sv_costhetasvpv"] = np_acc_float()
        self._accumulator["tree"]["FatJet_sv_d3dsig"] = np_acc_float()
        self._accumulator["tree"]["FatJet_sv_deltaR"] = np_acc_float()
        self._accumulator["tree"]["FatJet_sv_dxysig"] = np_acc_float()
        self._accumulator["tree"]["FatJet_sv_enration"] = np_acc_float()
        self._accumulator["tree"]["FatJet_sv_mass"] = np_acc_float()
        self._accumulator["tree"]["FatJet_sv_normchi2"] = np_acc_float()
        self._accumulator["tree"]["FatJet_sv_ntracks"] = np_acc_float()
        self._accumulator["tree"]["FatJet_sv_phirel"] = np_acc_float()
        self._accumulator["tree"]["FatJet_sv_pt"] = np_acc_float()
        self._accumulator["tree"]["FatJet_sv_ptrel"] = np_acc_float()
    ...
    def process(self, events):
        ...
        fatjets['pfcandIdx'] = match_fatjet_pfcand(fatjets, events.FatJetPFCands, ak.ArrayBuilder()).snapshot()
